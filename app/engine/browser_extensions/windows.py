...
CHROMIUM_BROWSERS = {
    "chrome": Path("~/AppData/Local/Google/Chrome/User Data"),
    "edge": Path("~/AppData/Local/Microsoft/Edge/User Data"),
    "brave": Path("~/AppData/Local/BraveSoftware/Brave-Browser/User Data"),
    "opera": Path("~/AppData/Roaming/Opera Software/Opera Stable"),
}

FIREFOX_PATH = Path("~/AppData/Roaming/Mozilla/Firefox/Profiles")
...
def score_manifest(manifest: Dict[str, Any], ext_name: str) -> Tuple[List[Dict[str, Any]], int]:
    ...
    trusted = any(hint in low_name for hint in TRUSTED_EXTENSION_HINTS)
    ...
    if isinstance(update_url, str) and not any(domain in update_url for domain in KNOWN_UPDATE_DOMAINS):
    ...
    if combo_score:
        findings.append(make_finding("risk_combo", f"Risky combination present: {', '.join(combo_reasons)}", combo_score))
        total += combo_score
...
def chromium_profiles(base: Path) -> List[Tuple[str, Path]]:
    ...
    for browser, base in CHROMIUM_BROWSERS.items():
        for profile_name, profile_path in chromium_profiles(expand(base)):
            ...
            for ext_id_dir in ext_root.iterdir() if ext_root.exists() else []:
                ...
                versions = sorted([p for p in ext_id_dir.iterdir() if p.is_dir()], key=lambda p: p.name, reverse=True)
                if not versions:
                    continue
                ...
                if total >= 10:
                    findings.append(make_finding("browser_extension", f"High-risk extension: {item['name']} ({browser})", 8, {"id": item["id"], "path": item["path"]}))
                elif total >= 5:
                    findings.append(make_finding("browser_extension", f"Review extension: {item['name']} ({browser})", 4, {"id": item["id"], "path": item["path"]}))
